...
fix(api): correct AQL query             # → patch bump
feat!: redesign API response format     # → major bump (breaking)
```
...
### Version Locations
- `web/package.json` — frontend version (injected at build time via `__APP_VERSION__`)
- `api/pyproject.toml` — API version (read via `importlib.metadata`)
- `api/src/main.py` — FastAPI `app.version` (also bumped by CI)
- `GET /api/version` — runtime version endpoint

### Changelog
`CHANGELOG.md` is auto-generated by [git-cliff](https://git-cliff.org/) from conventional commits. Do not edit manually.
